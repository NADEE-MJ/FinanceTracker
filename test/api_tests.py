...
for value in success:
    user = users[value]
    params = {"email": user.email, "password": user.password1}
...
for user in logged_in:
    for i in range(3):
        params = {"ticker": stocks[i].ticker, "access_token": users[user].access_token}
...
for user in logged_in:
    for i in range(3):
        params = {"symbol": cryptos[i].symbol, "access_token": users[user].access_token}
...
params[
    "access_token"
] = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJmcmVzaCI6dHJ1ZSwiaWF0IjoxNjQzNTY3MzczLCJqdGkiOiJhNj.4"
response = requests.get(base + "stocks", json=params)
print(response.json())
...
params[
    "access_token"
] = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJmcmVzaCI6dHJ1ZSwiaWF0IjoxNjQzNTY3MzczLCJqdGkiOiJhNj.s4sv-QqJhVq4cfu2cmhtB5X6P-i8ZyqEbUoKT94GswU"
response = requests.get(base + "stocks", json=params)
print(response.json())
...
params[
    "access_token"
] = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJmcmVzaCI6dHJ1ZSwiaWF0IjoxNjQzNTY3NTk1LCJqdGkiOiIzMDExZTQzOC0zN2YyLTRmMTEtOGQ0Yy1mYjYxNGE4ZjJmYTUiLCJ0eXBlIjoiYWNjZXNzIiwic3ViIjoidXNlcm5hbWUzMCIsIm5iZiI6MTY0MzU2NzU5NSwiZXhwIjoxNjQzNTY5Mzk1fQ.lB0eR_azUVlrvYH92cWBT3CKJWKejzrIVP2y31_9MTk"
response = requests.get(base + "stocks", json=params)
print(response.json())
...
"""
for user in logged_in:
    params = {"access_token": users[user].access_token}
    response = requests.get(base + "stocks", json=params)
    print(response.json())
...
"""
for user in logged_in:
    params = {"refresh_token": users[user].refresh_token}
